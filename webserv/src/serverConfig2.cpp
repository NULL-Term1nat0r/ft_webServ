...
serverConf::serverConf(Config conf) : _workerProcesses(1), _workerConnections(1), _scriptTimeout(0), _clientTimeout(0), _buffSize(9000), _backlog(1),
	...
	constructStatusMessageContainer();
	getServerConf(conf);
}
...
void	serverConf::_setScriptTimeouts(std::map<std::string, std::vector<std::string> > globalContext) {
	if (globalContext.find("script_timeout") != globalContext.end() && globalContext["script_timeout"].size() > 0)
...
void	serverConf::_setWorkerConnections(std::map<std::string, std::vector<std::string> > globalContext) {
	if (globalContext.find("worker_connections") != globalContext.end() && globalContext["worker_connections"].size() > 0)
...
void	serverConf::_setWorkerProcesses(std::map<std::string, std::vector<std::string> > globalContext) {
	if (globalContext.find("worker_processes") != globalContext.end() && globalContext["worker_processes"].size() > 0)
...
void	serverConf::_setPort(std::map<std::string, std::vector<std::string> > location, serverSettings &conf) {
	if (location.find("listen") != location.end() && location["listen"].size() > 0)
...
void	serverConf::_setBuffSize(std::map<std::string, std::vector<std::string> > globalContext) {
	if (globalContext.find("buff_size") != globalContext.end() && globalContext["buff_size"].size() > 0)
...
void	serverConf::_setBodySize(std::map<std::string, std::vector<std::string> > location, serverSettings &conf) {
	if (location.find("body_size") != location.end() && location["body_size"].size() > 0)
...
void	serverConf::_setServerName(std::map<std::string, std::vector<std::string> > location,serverSettings &conf) {
	if (location.find("server_name") != location.end() && location["server_name"].size() > 0)
