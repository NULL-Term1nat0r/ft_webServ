...
std::string cgiRequest::parseGetRequestPage() {
	...
	std::vector<std::string> result = parsing::split(_baseRequest->url, '/');
	return constructScriptPage(result);
}
...
void cgiRequest::executeCgi() {
	...
	pid_t childId = fork();
	if (childId == -1) {
		std::cout << "chile didnt got created\n";
		...
		if (chdir(_workingDirectory.c_str()) != 0){
			std::cout << blue << "chdir didnt work: " << _workingDirectory << reset << std::endl;
			exit(69);
		}
